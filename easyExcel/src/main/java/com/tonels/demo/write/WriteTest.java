...
import com.tonels.util.TestFileUtil;
...
public class WriteTest {
    ...
    public void repeatedWrite1() {
        ...
        WriteSheet writeSheet = EasyExcel.writerSheet("方法1模板").build();
    ...
    public void repeatedWrite() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            WriteSheet writeSheet = EasyExcel.writerSheet(i, "模板"+(i+1)).head(DemoData.class).build();
            // 分页去数据库查询数据 这里可以去数据库查询每一页的数据
            List<DemoData> data = data();
            excelWriter.write(data, writeSheet);
        }
    ...
    // 自定义拦截器，单元格的下拉框等等
    @Test
    public void customHandlerWrite2() {
        String fileName = TestFileUtil.getPath() + "customHandlerWrite" + System.currentTimeMillis() + ".xlsx";
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, DemoData.class)
                .registerWriteHandler(new CustomSheetWriteHandler2())
